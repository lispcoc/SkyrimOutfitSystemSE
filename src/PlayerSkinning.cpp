...
namespace OutfitSystem {
    ...
    namespace DontVanillaSkinPlayer {
        bool _stdcall
        ShouldOverride(RE::TESObjectARMO
        * armor,
        RE::TESObjectREFR *target
        ) {
        if (!
        ShouldOverrideSkinning(target)
        ) return false;
...
void Apply() {
    ...
    LOG(info,
        "TESObjectARMO_ApplyArmorAddon = {:x}",
...
namespace ShimWornFlags {
    ...
    void Apply() {
        ...
        LOG(info,
            "InventoryChanges_GetWornMask = {:x}",
            InventoryChanges_GetWornMask.address() - REL::Module::get().base());
        {
            ...
            LOG(info,
                "AVI: Patching shim worn flags at addr = {:x}. base = {:x}",
                ShimWornFlags_Hook,
                REL::Module::get().base());
    ...
    void Apply() {
        {
            ...
            LOG(info,
                "AVI: Patching custom skin player at addr = {}. base = {}",
                CustomSkinPlayer_Hook,
                REL::Module::get().base());
...
namespace FixEquipConflictCheck {
    ...
    void Apply() {
        ...
        LOG(info,
            "BGSBipedObjectForm_TestBodyPartByIndex = {:x}",
            BGSBipedObjectForm_TestBodyPartByIndex.address() - REL::Module::get().base());
        {
            ...
            LOG(info,
                "AVI: Patching fix equip conflict check at addr = {:x}. dst = {}. base = {:x}",
                FixEquipConflictCheck_Hook,
                code,
                REL::Module::get().base());
...
namespace RTTIPrinter {
    ...
    void Apply() {
        ...
        LOG(info,
            "InventoryChanges_ExecuteVisitorOnWorn_Hook = {:x}",
            InventoryChanges_ExecuteVisitorOnWorn_Hook - REL::Module::get().base());
        {
            ...
            LOG(info,
                "AVI: Patching RTTI print at addr = {:x}. base = {:x}",
                InventoryChanges_ExecuteVisitorOnWorn_Hook,
                REL::Module::get().base());
